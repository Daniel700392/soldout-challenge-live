...
const createBookingSaga = async (bookingData) => {
    const { userId, eventId, quantity, requestId } = bookingData;
    ...
    const newBooking = await db.query(
        'INSERT INTO bookings (user_id, event_id, quantity, request_id, status) VALUES ($1, $2, $3, $4, $5) RETURNING *',
        [userId, eventId, quantity, requestId, 'PENDING']
    );
    ...
    try {
        ...
        try {
            const paymentResult = await paymentClient.processPayment(booking.id, 150);
